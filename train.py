...
import os
import natsort
from evaluate_seg import ConfusionMatrix
...
class reverse_huber_loss(nn.Module):
    def __init__(self):
        super(reverse_huber_loss, self).__init__()
...
class trainer:
    def test(self, data_loader_test):
        ...
        net = natsort.natsorted(net)[-1]
        model.load_state_dict(torch.load(config["load_network_path"] + "/" + net))
        with torch.set_grad_enabled(False):
            model.eval()
            prediction = {}
            gt = {}
            for i, output in enumerate(data_loader_test):
                if i > 30:
                    break
                img = output.get("img")
                seg = output.get("seg")
                ...
                gt[i] = target
                ############################## evaluate-SEG ##################################
                out_seg = F.softmax(out_seg, dim=1)  # size (1, 34, 128, 256)
                out_seg = torch.argmax(out_seg, dim=1)  # size = (1, 128, 256)
                print(seg.shape, out_seg.shape)
                eval_seg = ConfusionMatrix(out_seg, seg)
                eval_seg.construct()
                mIOU = eval_seg.computeMiou()
                print(" Here is mIOU!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", mIOU)
                ############################## evaluate-SEG ##################################
                ############################## evaluate-depth ##################################
                abs_rel, sq_rel, rmse, rmse_log, a1, a2, a3 = evaluate_depth(
                    out_depth, depth
                )
                print("depth error:", abs_rel, sq_rel, rmse, rmse_log, a1, a2, a3)
                ...
                imgs = [seg, out_seg, img, img]
                utils.visualize(imgs, "./test", bboxes, target, 1, 1)
                ############################## Visualize ##################################
    ...
    def train(self, data_loader_train, data_loader_val):
        ...
        for epoch in range(params.num_epoch):
            ...
            for idx, output in enumerate(data_loader_train):
                ...
                # out_seg = F.log_softmax(out_seg, dim=1)  # size (1, 34, 128, 256)
                # out_seg = torch.argmax(out_seg, dim=1)  # size = (1, 128, 256)
                Loss_seg = criterion(
                    out_seg, seg.long()
                )  # out_seg = [1,128,256], seg = [1,127,256] (0~33)
