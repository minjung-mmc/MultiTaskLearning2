...
def default_detection_model_configs():
    ...
    h.backbone_name = "tf_efficientnet_b0"
    ...
    h.pad_type = (
        "same"
    )  # original TF models require an equivalent of Tensorflow 'SAME' padding
    h.act_type = "swish"
    ...
    h.redundant_bias = (
        True
    )  # original TF models have back to back bias + BN layers, not necessary!
    h.head_bn_level_first = (
        False
    )  # change order of BN in head repeat list of lists, True for torchscript compat
    h.head_act_type = None  # activation for heads, same as act_type if None

    h.fpn_name = None
    h.fpn_config = None
    h.fpn_drop_path_rate = (
        0.0
    )  # No stochastic depth in default. NOTE not currently used, unstable training
    ...
    h.label_smoothing = (
        0.0
    )  # only supported if legacy_focal == False, haven't produced great results
    h.legacy_focal = (
        False
    )  # use legacy focal loss (less stable, lower memory use in some cases)
    h.jit_loss = (
        False
    )  # torchscript jit for loss fn speed improvement, can impact stability and/or increase mem usage
