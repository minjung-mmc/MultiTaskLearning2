...
def decode_box_outputs(rel_codes, anchors, output_xyxy: bool = False):
    ...
    # print(" max tx in decode_box_outputs:  ", torch.max(tx), " min tx in decode_box_outputs:  ", torch.min(tx))
    ...
    xmax = xcenter + w / 2.0
    if output_xyxy:
        out = torch.stack([xmin / 8, ymin / 8, xmax / 8, ymax / 8], dim=1)
    else:
        out = torch.stack([ymin / 8, xmin / 8, ymax / 8, xmax / 8], dim=1)

    # print(" max x and min y in decode_box_outputs:  ", xmax/8, xmin/8)
    return out
...
class Anchors(nn.Module):  # 절대좌표
    ...
    def __init__(
        ...
        print("img__size", image_size)  # 1024, 2048 # from config file in train.py
