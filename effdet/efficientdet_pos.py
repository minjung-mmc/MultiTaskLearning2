...
class EfficientDet(nn.Module):
    def __init__(
        ...
        self.decoder1 = Decoder(n_classes_seg, feature_info[4]["num_chs"], batchnorm)
        self.decoder2 = Decoder(n_classes_depth, feature_info[4]["num_chs"], batchnorm)

        self.class_net = HeadNet(self.config, num_outputs=self.config.num_classes)
        self.box_net = HeadNet(self.config, num_outputs=4)
        ft = [
            feature_info[0]["num_chs"],
            feature_info[1]["num_chs"],
            feature_info[2]["num_chs"],
        ]
        self.attn = multi_attention(ft)
...
class Decoder(nn.Module):
    def __init__(self, n_classes, channel_num, batchnorm=False):
        super(Decoder, self).__init__()
        self.up1 = up(channel_num * 2, channel_num, batchnorm)
        self.up2 = up(channel_num * 2, channel_num, batchnorm)
        self.up3 = up(channel_num * 2, channel_num, batchnorm)
        self.up4 = up(channel_num * 2, channel_num, batchnorm)
        self.outc = outconv(channel_num, n_classes)
...
class positional_encoding(nn.Module):
    ...
    def forward(self, inputs):
        ...
        position_enc = np.array([[pos / np.power(10000, (i-i%2)/C) for i in range(C)]
        for pos in range(N)])

        position_enc[:,0::2] = np.sin(position_enc[:,0::2])
        position_enc[:,1::2] = np.cos(position_enc[:,1::2])

        position_enc = torch.from_numpy(position_enc).float().to(device=params.device) #[N,C]

        return position_enc.unsqueeze(0).repeat(B,1,1)
...
class attention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        positional_enc = self.positional_enc(q_p.reshape(B, -1, C))

        q_p, k_p, v_p = (
            self.linear_q(q_p.reshape(B, -1, C)+positional_enc),
            self.linear_k(k_p.reshape(B, -1, C)+positional_enc),
            self.linear_v(v_p.reshape(B, -1, C)+positional_enc)
        )

        q_p = torch.cat(
            torch.tensor_split(q_p, self.head_num, dim=-1), dim=0
        )  # [B*num_head, H*W, C/num_head]
        k_p = torch.cat(
            torch.tensor_split(k_p, self.head_num, dim=-1), dim=0
        )  # [B*num_head, H*W, C/num_head]
        v_p = torch.cat(
            torch.tensor_split(v_p, self.head_num, dim=-1), dim=0
        )  # [B*num_head, H*W, C/num_head]

        attention, attention_score = self.scaled_dot_product_attention(
            q_p, k_p, v_p, mask
        )
        # attention score : [B*num_head, H*W, H*W]

        output = torch.cat(
            torch.tensor_split(attention, self.head_num, dim=0), dim=-1
        )  # [B, H*W, C]
        output = self.linear_o(output)

        output_re = output.permute(0, 2, 1).reshape(B, C, H, W)

        return output_re, attention_score
...
class semi_attention(nn.Module):
    ...
    def __init__(self, in_feature, head_num, bias=True, activation=F.relu):
        super(semi_attention, self).__init__()
    ...
    def forward(self, attention_map, v, mask=None):

        B, C, H, W = v.shape

        v_p = v.permute(0, 2, 3, 1)

        v_p = v_p.reshape(B,-1,C)
        # v_p = self.linear_v(v_p.reshape(B, -1, C))

        v_p = torch.cat(
            torch.tensor_split(v_p, self.head_num, dim=-1), dim=0
        )  # [B*num_head, H*W, C/num_head]

        attention = attention_map.matmul(v_p)
        # attention score : [B*num_head, H*W, H*W]
...
class multi_attention(nn.Module):
    def __init__(self, feature_size):
        ...
        # self.attention_net = [
        #     attention(feature_size[i], head_num=4) for i in range(len(feature_size))
        # ]
        self.semi_attention_net = [
            semi_attention(feature_size[i], head_num=4) for i in range(len(feature_size)-1)
        ]
        self.attention_net = attention(feature_size[-1], head_num=4)
    ...
    def forward(self, x):
        ...
        out_x3, attention_map = self.attention_net(x[2], x[2], x[2])

        # print(attention_map.shape, x[2].shape)
        # print(attention_map.unsqueeze(0).shape)
        # # attention_map_up1 = self.upsample(attention_map.unsqueeze(0)).squeeze(0)

        
        # # print(attention_map_up1.shape, x[1].shape)

        # out_x2 = self.semi_attention_net[1](self.upsample1(attention_map.unsqueeze(0)).squeeze(0), x[1])
        

        # # print(attention_map_up2.shape, x[0].shape)

        # out_x1 = self.semi_attention_net[0](self.upsample2(attention_map.unsqueeze(0)).squeeze(0), x[0])


        # print("test for x1 : ", [x[0].shape, out_x1.shape])
        
        # print("test for x2 : ", [x[1].shape, out_x2.shape])
        
        # print("test for x3 : ", [x[2].shape, out_x3.shape])


        # return [self.attention_net[i](x[i],x[i],x[i]) for i in range(len(x))]
        return [x[0], x[1], out_x3]
